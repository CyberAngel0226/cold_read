import assert from "node:assert/strict";
import test from "node:test";

import {
  adaptMarketStructureDraft,
  type EvidenceSnapshot,
  type MarketStructureRecommendationDraft,
} from "../src/index.js";

const evidenceSnapshot: EvidenceSnapshot = {
  id: "snapshot_1",
  decisionRunId: "run_1",
  createdAt: "2026-06-10T00:04:00.000Z",
  marketEvidence: {
    screenedMarkets: [
      {
        id: "screened_candidate_poly_1",
        polymarketId: "poly_1",
        question: "Will the Fed cut rates in July?",
        outcomes: ["YES", "NO"],
        prices: {
          yes: 0.92,
          no: 0.08,
        },
        volume: 100000,
        liquidity: 25000,
        closeTime: "2026-07-10T00:00:00.000Z",
        resolutionRules: "Resolves YES if the Fed cuts rates in July.",
        oneSidedSignal: {
          side: "YES",
          price: 0.92,
          rationale: "YES price is strongly one-sided.",
        },
        confirmationRationale: "Tavily context found no major counterevidence.",
      },
    ],
  },
  contextEvidence: {
    items: [],
  },
};

test("adapts a Python Market Structure BUY_YES draft into a domain Agent Recommendation", () => {
  const draft: MarketStructureRecommendationDraft = {
    action: "BUY_YES_SMALL",
    targetMarketId: "screened_candidate_poly_1",
    rationale: "Market structure shows a liquid one-sided YES signal.",
    confidence: "MEDIUM",
    riskLevel: "LOW",
    evidenceRefs: ["screened_candidate_poly_1"],
  };

  const recommendation = adaptMarketStructureDraft({
    evidenceSnapshot,
    draft,
    now: "2026-06-10T00:05:00.000Z",
    createRecommendationId: () => "rec_market_structure_1",
    createWalletActionProposalId: () => "wallet_action_1",
    smallStakeAmount: "5.00",
  });

  assert.deepEqual(recommendation, {
    id: "rec_market_structure_1",
    decisionRunId: "run_1",
    evidenceSnapshotId: "snapshot_1",
    analysisLens: "MARKET_STRUCTURE",
    action: "BUY_YES_SMALL",
    targetMarketId: "screened_candidate_poly_1",
    walletActionProposal: {
      id: "wallet_action_1",
      marketId: "screened_candidate_poly_1",
      action: "BUY_YES_SMALL",
      stake: {
        amount: "5.00",
        currency: "USDC",
      },
      rationale: "Market structure shows a liquid one-sided YES signal.",
    },
    rationale: "Market structure shows a liquid one-sided YES signal.",
    confidence: "MEDIUM",
    riskLevel: "LOW",
    evidenceRefs: ["screened_candidate_poly_1"],
    createdAt: "2026-06-10T00:05:00.000Z",
  });
});

test("adapts a Python Market Structure BUY_NO draft into a buy recommendation", () => {
  const recommendation = adaptMarketStructureDraft({
    evidenceSnapshot,
    draft: {
      action: "BUY_NO_SMALL",
      targetMarketId: "screened_candidate_poly_1",
      rationale: "NO side is the one-sided market-structure signal.",
      confidence: "MEDIUM",
      riskLevel: "MEDIUM",
      evidenceRefs: ["screened_candidate_poly_1"],
    },
    now: "2026-06-10T00:05:00.000Z",
    createRecommendationId: () => "rec_market_structure_2",
    createWalletActionProposalId: () => "wallet_action_2",
    smallStakeAmount: "5.00",
  });

  assert.equal(recommendation.action, "BUY_NO_SMALL");
  assert.equal(recommendation.targetMarketId, "screened_candidate_poly_1");
  assert.deepEqual(recommendation.walletActionProposal, {
    id: "wallet_action_2",
    marketId: "screened_candidate_poly_1",
    action: "BUY_NO_SMALL",
    stake: {
      amount: "5.00",
      currency: "USDC",
    },
    rationale: "NO side is the one-sided market-structure signal.",
  });
});

test("adapts a Python Market Structure HOLD draft without a wallet action", () => {
  const recommendation = adaptMarketStructureDraft({
    evidenceSnapshot,
    draft: {
      action: "HOLD",
      rationale: "Market structure is not strong enough for a small position.",
      confidence: "LOW",
      riskLevel: "HIGH",
      evidenceRefs: ["screened_candidate_poly_1"],
    },
    now: "2026-06-10T00:05:00.000Z",
    createRecommendationId: () => "rec_market_structure_3",
    createWalletActionProposalId: () => "wallet_action_3",
    smallStakeAmount: "5.00",
  });

  assert.deepEqual(recommendation, {
    id: "rec_market_structure_3",
    decisionRunId: "run_1",
    evidenceSnapshotId: "snapshot_1",
    analysisLens: "MARKET_STRUCTURE",
    action: "HOLD",
    rationale: "Market structure is not strong enough for a small position.",
    confidence: "LOW",
    riskLevel: "HIGH",
    evidenceRefs: ["screened_candidate_poly_1"],
    createdAt: "2026-06-10T00:05:00.000Z",
  });
});

test("rejects malformed Python Market Structure drafts before they become domain recommendations", () => {
  const baseInput = {
    evidenceSnapshot,
    now: "2026-06-10T00:05:00.000Z",
    createRecommendationId: () => "rec_market_structure_4",
    createWalletActionProposalId: () => "wallet_action_4",
    smallStakeAmount: "5.00",
  };

  assert.throws(
    () =>
      adaptMarketStructureDraft({
        ...baseInput,
        draft: {
          action: "BUY_YES_SMALL",
          rationale: "Buy draft must name a target market.",
          confidence: "MEDIUM",
          riskLevel: "LOW",
          evidenceRefs: ["screened_candidate_poly_1"],
        },
      }),
    /Buy recommendations require a target market\./,
  );
  assert.throws(
    () =>
      adaptMarketStructureDraft({
        ...baseInput,
        draft: {
          action: "HOLD",
          targetMarketId: "screened_candidate_poly_1",
          rationale: "Hold draft cannot target a market.",
          confidence: "LOW",
          riskLevel: "HIGH",
          evidenceRefs: ["screened_candidate_poly_1"],
        },
      }),
    /HOLD recommendations cannot target a market\./,
  );
  assert.throws(
    () =>
      adaptMarketStructureDraft({
        ...baseInput,
        draft: {
          action: "BUY_YES_SMALL",
          targetMarketId: "screened_candidate_poly_1",
          rationale: "Evidence refs must come from the snapshot.",
          confidence: "MEDIUM",
          riskLevel: "LOW",
          evidenceRefs: ["unknown_market"],
        },
      }),
    /Market Structure evidenceRefs must reference snapshot market evidence\./,
  );
  assert.throws(
    () =>
      adaptMarketStructureDraft({
        ...baseInput,
        draft: {
          action: "BUY_MAYBE_LARGE",
          targetMarketId: "screened_candidate_poly_1",
          rationale: "Python drafts cannot invent actions.",
          confidence: "MEDIUM",
          riskLevel: "LOW",
          evidenceRefs: ["screened_candidate_poly_1"],
        } as unknown as MarketStructureRecommendationDraft,
      }),
    /Unsupported Market Structure action\./,
  );
});
